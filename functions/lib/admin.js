...
export const adminLogin = onCall({
    ...
    const fails = (rlSnap.data()?.fails ?? []).filter((t) => t > now - WINDOW_MS);
    if (fails.length >= MAX_FAILS) {
        const waitMin = Math.ceil((fails[0] + WINDOW_MS - now) / 60000);
        throw new HttpsError("resource-exhausted", `로그인 시도가 너무 많습니다. ${waitMin}분 후 다시 시도해 주세요.`);
    }
    ...
    try {
        ...
        throw new HttpsError("internal", "관리자 토큰 발급에 실패했습니다. 서비스 계정에 'Service Account Token Creator' 권한이 필요합니다.");
    }
